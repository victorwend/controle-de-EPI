...
1. ~~**Modal de sucesso duplicado para Estoque.**~~ **Resolvido em 07/09/2026.** `05.04 — Transferência concluída` e `11.05 — Estados — Transferência concluída` tinham o mesmo título e função, layouts diferentes. Decisão aplicada conforme a recomendação: construído `SuccessModal.jsx`, o componente genérico do módulo 11 (ícone de check, título, descrição, botão "Fechar"), usado como o estado de sucesso da Transferência de estoque; `05.04` foi descartada. Também corrigido um copiar-colar do Figma: a descrição do modal de sucesso da Transferência dizia "A transferência **do funcionário** foi registrada com sucesso" (texto claramente reaproveitado de um modal de Entregas) — substituído por "A transferência de estoque foi registrada com sucesso."
...
3. ~~**Cadastro de biometria tem dois pontos de entrada não unificados.**~~ **Resolvido em 07/09/2026.** Pode-se cadastrar biometria dentro de `04.02 — Novo funcionário` (inline) OU via `09.10–09.13` (fluxo dedicado a partir de Cadastros). Decisão: um único componente (`BiometriaCadastro.jsx`) com estado interno de passo (busca → captura → confirmar → sucesso), reaproveitado pelos dois pontos de entrada. Quando chega pela busca dedicada (09.10), começa no passo de busca; quando chega pelo atalho de `04.02`, recebe o funcionário já selecionado via router state e pula direto para a captura. Os textos de "voltar" no fim do fluxo mudam conforme a origem. Testado ponta a ponta nos dois caminhos.
...
6. ~~**`05.05 — Bloqueio por saldo insuficiente` é chamada a partir de Entregas, não de Estoque**~~ **Resolvido em 07/09/2026.** Implementado como um passo interno do wizard de `NovaEntrega.jsx` (Sprint 8): ao revisar a entrega, qualquer item com quantidade acima do saldo dispara o estado "Estoque insuficiente" com a lista de itens com problema, replicando o conteúdo exato de `05.05`. Testado ponta a ponta contra o saldo mock (`SALDO_POR_LOCAL`), inclusive o caminho de ajustar a entrega e prosseguir depois de corrigir a quantidade.
